...
// Global variables.
const ORG = 'extra-entries';
const ORIGINAL = 'Original module: [extra-entries].';
...
function pkgMinify(o) {
  ...
  readme = readme.replace(/(\.<br>)/, ', exported as `Entries`$1');
  readme = readme.replace(/(\.<br>)[\s\S]*?(\[!\[nodef\])/, `$1${EOL}${ORIGINAL}${EOL}${CDN}${EOL}<br>${EOL}${EOL}${EOL}$2`);
  readme = readme.replace(/extra-entries\.min/g, 'extra-entries');
  readme += `[unpkg]: https://unpkg.com/extra-entries.min${EOL}`;
  readme += `[jsDelivr]: https://cdn.jsdelivr.net/npm/extra-entries.min${EOL}`;
  pkg.name += '.min';
